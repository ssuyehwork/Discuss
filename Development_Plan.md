...
## [2026-10-24] 职责单一原则 (SRP) 全量排查、建档与标记审查

- 用户描述的现象/问题：当前代码库中存在可能违反“职责单一原则 (SRP)”与“模块化标准”的类、函数和模块。
- 用户期望的结果：对全量代码库执行系统性的 SRP 违规扫描与排查，将每一处违规以统一的高质量格式记录进专门维护的 `ARCHITECTURE_DEBT.md` 文档中，不执行任何实际代码修改。
- 本次任务边界：逐一排查项目中的所有类及核心函数，对照 God Object / God Function、绘制/渲染层职责过载、数据层与业务层混杂、反向依赖/越级耦合、历史“伪重构”等五大违规判定标准，穷举当前职责并提出拆分解耦方案，写入 `ARCHITECTURE_DEBT.md` 并根据优先级排序。
- 不在本次范围内的是：修改任何 `.cpp` / `.h` 等源文件或执行代码重构。
- 对应方案文档：无（纯排查建档分析任务，产出为 ARCHITECTURE_DEBT.md）
