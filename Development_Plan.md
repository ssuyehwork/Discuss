...
## [2026-07-24] 最新主干物理职责过载实地排查与大纲共识

- 用户描述的现象/问题：当前主干版本（234d520）核心类和业务流程中依然面临着显著的非单一职责侵入。
- 用户期望的结果：完全抛弃历史遗留结论，根据当前代码实况，深入物理持久层、UI层和控件交互，揪出真实行号之下的不合规职责过载，并达成明确的解耦大纲共识。
- 本次任务边界：实地审计并建档记录：`DatabaseManager` 在 `loadDb` 中越权操作物理文件隐藏效果；`TagManagerView` 作为视图直接在 `addTagToGroup` 等后台线程操作写库；`ContentPanel` 现场混入 3 遍随机覆写等重型安全删除算法。
- 不在本次范围内的是：修改任何 `.cpp`/`.h` 源码文件，或执行编译。
- 对应方案文档：ARCHITECTURE_DEBT.md

## [2026-07-24] 58号方案反败事高并发技术阻断与防护边界

- 用户描述的现象/问题：任何重构图纸在实施阶段都容易因为忽视并发冲突、对象生命周期及大文件 IO 而导致崩溃与假死等“败事”现象。
- 用户期望的结果：制定最硬核的系统级技术限制与红线。在 58 号重构开始前，对可能出现的内存踩踏、信号静默丢失以及 IO 线程抢占进行强制对账。
- 本次任务边界：设计防崩溃防护标准：`TagManagerController` 绝对使用 `QPointer` 及信号槽，View 析构自切断连接防野指针踩踏；跨线程只传输 Qt 基础类型，免去元类型未注册丢失隐患；大文件擦除追加 1ms 循环 yield 保护，为前台 I/O 腾出带宽。
- 不在本次范围内的是：修改任何功能代码。
- 对应方案文档：Modification_Plan/Modification_Plan-58.md
