...
- 用户期望的结果：
  1. 在已经进入编辑状态后，按下向上/向下键应被完全屏蔽拦截，绝对不向上 or 向下游动选中邻近项目。
...
## [2026-07-30] 内容面板右键菜单选项双轨强隔离分流重构

- 用户描述的现象/问题：内容面板右键菜单集成了 25 个选项。其中有 7 个核心写操作与物理交互选项，由于缺少对 DataSourceType 双轨数据来源的精准判定和系统级分流，极易造成写数据溢流到非预期路由中，污染 SQLite 数据库或造成物理文件错误。
- 用户期望的结果：确保设定颜色、单项重命名、批量重命名、删除、归类与迁移、粘贴、刷新这 7 个选项根据托管库模式与磁盘模式进行强隔离的安全判定与逻辑分流；且在磁盘模式下，重命名需保持缓存平滑自愈，并在加载及操作时自动对 JSON 孤立条目进行清理，达成真正自给自足、0% 漏水污染的双轨极速体验。
- 本次任务边界：在 `src/ui/ContentPanel.cpp` 中重构 `onCustomContextMenuRequested` 以及重命名 `performBatchRename`、物理删除、粘贴 `performPaste` 逻辑。针对 7 个分流核心选项：设定颜色时在托管库模式存入 SQLite、在磁盘模式写离散 `AmMetaJson`（绝对不入 SQL）；单项/批量重命名时在托管库改 SQLite、磁盘模式改物理文件并更新 JSON Key 同时执行平滑缓存 key 迁移；删除时托管库执行逻辑删除或解绑、磁盘模式执行系统回收/物理抹除并清理 JSON 项目；粘贴在托管库调用打包封装、磁盘模式调用 OS 级复制并按需加载；刷新在托管库查库、磁盘模式物理重扫。在载入和刷新时增加物理孤立条目自动清理。
- 不在本次范围内的是：修改物理 MFT 扫描、USN 监控底座或数据库相关的非界面菜单逻辑。
- 对应方案文档：Modification_Plan/Modification_Plan-9.md
