...
- 本次任务边界：修改 `src/ui/TreeItemDelegate.h` 中 Column 0 处于 `m_drawMiniCards` 状态时的图片/图标自绘逻辑。打通 `QIcon` 缓存的 QPixmap 提取与平滑缩放绘制链路，保证即使模型返回 QIcon 类型的缓存，也能够自动、高品质地缩放以填满 4px 圆角微型卡片容器。
