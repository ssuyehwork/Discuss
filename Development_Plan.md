...
## [2026-07-26] main.cpp 打补丁式傻逼逻辑架构排查与极致重构规划

- 用户描述的现象/问题：`main.cpp` 充满了打补丁式编程，存在严重的单实例锁、日志高并发同步写性能瓶颈、COM未成对且过早初始化冲突、退出时无优雅清理与强杀线程致数据库易损、以及 UI 构造与后台服务启动顺序倒置导致的渲染竞争等 6 大核心架构败笔。
- 用户期望的结果：对 `main.cpp` 进行代码审计并给出详细的根因定位报告；同时规划重构该程序运行流程顺序与多线程退出、启动流程，让生命周期和资源回收达到专业工业级标准。
- 本次任务边界：
  1. 审计 `src/main.cpp` 源码，精确定位 6 大败笔的代码位置与技术成因，并输出审计报告。
  2. 提出一套清爽高内聚、支持内存队列与异步线程后台落盘、严格拓扑依赖预热、单实例强力哨兵与优雅优雅清理的 C++ 重构图纸方案。
  3. 因为当前是分析师角色，本 Turn 仅可产出 `Modification_Plan-89.md` 方案文档，坚决不修改任何功能性 C++ 代码文件。
- 不在本次范围内的是：在未获得用户明确批准执行授权时，修改 `.cpp`/`.h` 代码文件。
