...
- 不在本次范围内的是：重写或修改物理 USN 监控底座 or 底层 MFT 文件过滤搜索流程。
...
- 不在本次范围内的是：逐个修改各页面组件按钮 of 逻辑，或在其他样式表文件中写分散的规则。
...
## [2026-07-26] 全库地毯式代码质量、架构合理性与性能隐患深度审计

- 用户描述的现象/问题：全库可能存在补丁堆砌、职责错位（违反 SRP）、性能阻塞隐患以及线程/生命周期不安全等底层硬伤。
- 用户期望的结果：对全库 `src/` 下所有的 .h / .cpp 文件进行地毯式静态审查，并按照规定的四大维度特征进行定位，最终在项目根目录下输出极度详实专业的 `Architectural_Audit_Report.md`。
- 本次任务边界：
  1. 地毯式扫描 `src/` 下的文件，精确定位补丁规避（如 singleShot(0)、blockSignals、特定特殊字符流转、变通更新）、职责错位（UI直接删物理文件/COM/SQL、Model 读磁盘、main过载）、性能阻塞（主线程I/O、过度ResetModel、data高开销调用）及生命期不安全（单例跨线程哑死、无优雅退出、无Cancel/弱指针的裸起线程池）等问题。
  2. 将审计结果以汇总表格加逐个严重缺陷剖析的形式记录在 `Architectural_Audit_Report.md` 中。
  3. 绝不修改或编译任何 C++ 功能代码，严格遵守“分析师”角色边界。
- 不在本次范围内的是：实际修改代码或在外部运行测试。
- 对应方案文档：无（属于只读审计与方案规划委托，直接产出 `Architectural_Audit_Report.md`，不需要单独新建 Modification_Plan 并执行代码修改）
