...
- **菜单图标无彩色单色限制**: 全软件所有右键菜单与上下文菜单项的图标统一采用中性单色 (`#EEEEEE`)，严格禁止在右键菜单图标上使用任何彩色（如红、黄、蓝、绿等着色），确保右键菜单全局界面沉静统一、高档极简！
...
- **置顶按钮 (Pin Button)**:
  ...
  - **置顶实现规范**: 窗口置顶功能必须且只能采用 Windows 系统原生 API (`SetWindowPos` 配合 `HWND_TOPMOST` / `HWND_NOTOPMOST`) 实现，严禁使用重构 HWND 句柄引发闪烁的 `Qt::WindowStaysOnTopHint` 或自创非原生置顶控制逻辑。
- **同步按钮 (Sync Button)**:
...
- **窗口空白处拖拽与还原交互规范 (不可违背规则)**: 当主窗口处于最大化状态时，按住主窗口内任意非交互空白区域（包括标题栏及各面板背景空白处）拖拽移动窗口，必须统一触发“还原 + 移动”（Restore + Move）逻辑，先将窗口还原为常规尺寸并精确定位在光标位置下方，再跟随光标平滑移动；只有当窗口本身已处于还原/正常状态时，拖动空白处才执行纯移动（Move）。
