...
## 7. 目录导航栏回收站快捷入口与全流程交互规范 (NavPanel Trash Shortcut & Operations Specification)

### 7.1 设计理念
为了在纯磁盘模式下提供完整、直观的文件回收站管理体验，系统在第一栏目录导航栏 (`NavPanel`) 的顶部 Header 区域设立专享的“回收站”快捷按钮，并与内容面板 (`ContentPanel`) 及磁盘回收站服务 (`DiskTrashService`) 进行深层视图联动：

1. **顶栏快捷入口**：在目录导航栏顶部标题“目录导航”右侧放置精美回收站图标按钮，点击后将内容面板切换为回收站数据源 (`m_currentCategoryType = "trash"`)。
2. **专属右键菜单序列**：内容面板在回收站数据模式下，严格按照以下优先级与顺序构建右键菜单：
   - **【还原】**：原路秒级归位到删除前的原始物理目录。
   - **【剪切】**：将回收站中的文件/文件夹实际路径复制到剪切板，允许用户在任意正常磁盘目录执行“粘贴”，实现灵活的“恢复并搬家”。
   - **【永久删除】**：彻底抹除物理文件及关联数据库记录。
   - **【还原全部】**：一键归位回收站中的所有项目。
   - **【清空回收站】**：一键彻底清理并清空磁盘回收站。
