# Implementation Plan - MetaPanel Real Image Preview Feature Integration (Direction A)

## 1. Overview
Previously, `m_lblImagePreview` in `MetaPanel` was an uninitialized placeholder ("zombie code") that was always hidden and never assigned a pixmap when items were selected. As a result, selecting an image or vector file with a valid thumbnail left the top preview section empty/hidden in `MetaPanel`.

This implementation plan completes **Direction A** by:
1. Exposing `setImagePreview(const QPixmap& pixmap)` in `MetaPanel.h` / `MetaPanel.cpp`.
2. Updating `MetaPanel::adjustFlowHeights()` so that `m_topPreviewBox` dynamically calculates height and shows when either a preview pixmap or color palette pills exist.
3. Hooking up `MainWindow.cpp`'s selection change signal handler to read `HasThumbnailRole` and `Qt::DecorationRole` from `QModelIndex` and pass the image preview pixmap directly into `MetaPanel::setImagePreview()`.
...
3. Click on a file with a generated thumbnail and verify that the top of `MetaPanel` (`m_topPreviewBox`) now displays the real thumbnail image preview seamlessly.
