# Implementation Plan - FavoritePanel-6

This implementation plan refines `FavoritePanel` right-click context menus and header encapsulation. It replaces plain `QAction` lists with `QWidgetAction` combined with a `QGridLayout` (5 columns × 2 rows) for a 10-item vector SVG icon picker, embeds `ColorStripPicker` for color selection, routes button tooltips to `HoverEventFilter` / `ToolTipOverlay` (strictly forbidding native Qt tooltips), declares `saveFavorites()` as a private helper in `FavoritePanel.h`, and completely eliminates empty black right-hand whitespace margins.
...
- **`HoverEventFilter` Integration**: Replaces native `setToolTip` with `installEventFilter(new HoverEventFilter(label, btn))` for system-wide uniform tooltip rendering via `ToolTipOverlay`.
- **Private Method Encapsulation**: Declares `saveFavorites()` in `FavoritePanel.h` under `private:`, resolving MSVC compilation errors (`C3861`, `C2039`, `C2065`).
...
2. Launch QuarkMeta and test right-click actions in `FavoritePanel`:
   - Right-click a favorite folder item: Verify `ColorStripPicker` line and "切换图标" 5x2 grid icon menu appear with zero right-hand black empty space, and hover tooltips render cleanly via `ToolTipOverlay`.
