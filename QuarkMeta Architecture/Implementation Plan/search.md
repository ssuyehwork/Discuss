...
2. **Clean Encapsulation & Semantic Methods**:
   - Exposes semantic methods: `focusAndSelectAll()`, `clear()`, `keyword()`, and `historyPanel()` so external modules do not need raw `QLineEdit*` access.
   - `searchEdit()` accessor retained for optional direct widget access if required.
3. **Correct Service Methods**: Uses `SearchHistoryService::instance().appendSearch("global", keyword)` matching actual repository method signature.
...
5. `src/ui/MainWindow.cpp` (Delegate search toolbar creation and binding to `SearchController`, update search calls to `m_searchController`, remove `watched == m_searchEdit` from `eventFilter()`)
6. `src/ui/GlobalShortcutController.cpp` (Include `SearchController.h`, call `m_mainWindow->m_searchController->focusAndSelectAll()`)
7. `src/ui/PanelMediator.cpp` (Include `SearchController.h`, call `m_mainWindow->m_searchController->keyword()`)
...
void SearchController::focusAndSelectAll() {
    if (m_searchEdit) {
...
QString SearchController::keyword() const {
...
=======
    // 4. Ctrl+F: 聚焦搜索过滤框
    if (event->key() == Qt::Key_F && (event->modifiers() & Qt::ControlModifier)) {
        if (m_mainWindow->m_searchController) {
            m_mainWindow->m_searchController->focusAndSelectAll();
        }
        event->accept();
        return true;
    }
...
=======
    ...
    if (filterPanel && contentPanel) {
        connect(filterPanel, &FilterPanel::filterChanged, m_mainWindow, [this, contentPanel](const FilterState& state) {
            FilterState mergedState = state;
            if (m_mainWindow->m_searchController) {
                mergedState.keyword = m_mainWindow->m_searchController->keyword();
            }
            contentPanel->applyFilters(mergedState);
            m_mainWindow->updateStatusBar();
        });
...
6. Pressing Ctrl+F focuses search bar and selects all text via `focusAndSelectAll()`.
