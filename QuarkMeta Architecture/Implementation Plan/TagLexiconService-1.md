# TagLexiconService Implementation Plan (Revision 1)
...
This revision includes complete line-by-line Search/Replace diffs for UI components (`TagSelectorOverlay.cpp` and `TagManagerDialog.cpp`) and includes `#include <QDateTime>` in `TagLexiconService.cpp`.
...
- `src/core/TagLexiconService.cpp` (New implementation using native C SQLite API and QDateTime)
...
- `src/ui/TagSelectorOverlay.cpp` (Connect auto-completion and lexicon lookup to `TagLexiconService`)
...
### 3.5 `src/ui/TagSelectorOverlay.cpp`
```cpp
<<<<<<< SEARCH
...
=======
void TagSelectorOverlay::loadTagsAndGroups() {
    auto lexiconGroups = TagLexiconService::instance().getAllTagGroups();
    
    m_groupList->clear();
    m_groupList->addItem("全部");
    m_groupList->addItem("未分类");

    for (const auto& grp : lexiconGroups) {
        m_groupList->addItem("📁 " + grp.name);
    }
...
### 3.6 `src/ui/TagManagerDialog.cpp`
```cpp
<<<<<<< SEARCH
#include "../meta/TagRepository.h"
=======
...
1. Configure and build the CMake project to ensure `TagLexiconService` compiles without MOC or unresolved symbol errors.
2. Verify tag creation, rename, and deletion in `global.db` via `TagLexiconService` without triggering full-disk JSON file scanning.
