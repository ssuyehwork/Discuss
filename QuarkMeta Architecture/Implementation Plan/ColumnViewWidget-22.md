# Implementation Plan - ColumnViewWidget Fixed 220px Column Width & Blank Canvas Restoration
...
1. **破坏各列固定 220px 宽度设计理念**：当列数较少（1~3 列）时，算法强行将视口宽度均分，把每一列拉伸至 400px~500px 以上强行撑满屏幕；只有当列数极多撑破视口时，才会被挤压至下限；
...
- **各列列宽绝对固定为 220 像素**（`setFixedWidth(220)`），无论当前有 1 列、3 列还是 7 列，列宽永恒固定为 220px；
- **最右侧刻意留白画布（`ColumnBlankCanvasWidget`）弹性填充与保底机制**：
  ...
  - 当列总宽超出视口时（`totalPanesWidth >= viewportW`）：最右侧始终保持至少 220px 的刻意留白画布（`blankWidth = 220`），确保水平滚动条滑到最右端时，最后一列右侧永远保留一块 220px 宽度的空白区域供双击回退上一级与拖拽放置。
...
3. **根因溯源**：彻底清除 Gemini 擅自引入的 `viewportW / count` 伪均分算法，物理还原 `220px` 刚性列宽与留白保底公式。
...
| 文件路径 | 修改类型 | 说明 |
| :--- | :--- | :--- |
| `src/ui/ColumnViewWidget.cpp` | 逻辑修正 | 废除均分拉伸，锁定各列固定 220px 宽度，恢复刻意留白画布弹性填充与保底计算 |

---

## 3. Detailed Line-by-Line Changes（精准替换块）

### 3.1 `src/ui/ColumnViewWidget.cpp` — 还原各列固定 220px 宽度与刻意留白画布机制
...
1. **少列场景验证（1~2 列）**：
   ...
   - 验证每一列的宽度是否严格为 `220px`，绝无拉伸；
   - 验证最右侧是否呈现宽阔的刻意留白区域，右键/双击留白区域能够正常触发回退上一级或上下文菜单；
   - 验证无横向滚动条。
2. **多列溢出场景验证（7 列以上）**：
   - 逐级深入目录展开 7 列以上（如截图所示路径）；
   - 验证所有列的宽度保持一致的 `220px`；
   - 水平滚动条滚动至最右端，验证最后一列右侧是否存在一块明确的 `220px` 刻意留白画布，绝不再贴死在元数据栏边框上；
