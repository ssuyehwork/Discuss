...
namespace {
// 角色能力表——新增角色时，只需要在 WindowRole 枚举里加一项，再在这里补一行判断即可，
// 不需要改动 handleNativeEvent 里的具体消息处理逻辑。
...
FramelessWindowHelper::FramelessWindowHelper(QWidget* window, WindowRole role, QWidget* titleBar)
    ...
    if (caps.nativeChrome) {
        ...
        // 悬浮工具窗一类角色：只保留缩放所需的 WS_THICKFRAME，不注入标题栏/最大化/系统菜单语义
...
bool FramelessWindowHelper::handleNativeEvent(void* message, qintptr* result) {
    ...
    HWND hwnd = msg->hwnd;
    // 必须以 Win32 原生权威状态为唯一准绳，严禁使用状态滞后的 m_window->isMaximized()
    const bool isMax = ::IsZoomed(hwnd);
    const RoleCapabilities caps = capabilitiesFor(m_role);

    // 0. 抑制 Windows 默认背景擦除：消除 DWM 最大化/还原拉伸瞬间的黑框闪烁
    if (msg->message == WM_ERASEBKGND) {
        *result = 1;
        return true;
    }

    // 0b. 尺寸与位置变动原生分发：仅 Primary 角色回调标题栏同步最大化图标
    ...
    // 3. 原生 WM_NCHITTEST 精确命中检测——边缘缩放对所有角色一视同仁
    if (msg->message == WM_NCHITTEST) {
        ...
        // 标题栏原生拖拽与双击识别（排除交互控件）——仅 titleBarDrag 能力开启的角色生效
    ...
    // 5. 原生双击标题栏最大化 / 还原——仅 doubleClickMaximize 能力开启的角色生效
