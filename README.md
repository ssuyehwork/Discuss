...
## 4. 修改授权与 `Memories.md` 唯一真理源 (SSOT)
- 达成共识后，经用户需求与批准，才可以去更改 **“Memories.md”** 文档里的设计理念与规则。  
- 废弃说明：原 `QuarkMeta-Architecture-Planning.md` 和 `Guide & Preference.md` 已废弃且不适用，系统唯一真理源收敛至 **`Memories.md`**。

### 4.1 `Memories.md` 设计理念与记忆规则保护铁律
1. **职责绝对单一**：`Memories.md` **仅且只能记录应用的高级设计理念、顶层架构规划、UI规范与全局记忆**。
2. **严禁写入任何实施细节**：任何具体的代码修改点、Search/Replace Diff 替换块、具体代码行号、编译调试命令或局部修复方案，**绝对禁止写入 `Memories.md`**！
...
4. **最后修改** → 仅在用户批准后，且只改 “Memories.md”。  
...
补充说明：
1. Memories.md 只是用来记录设计理念与核心记忆，不可将相关实施方案记录到该文档里。
...
1. **强制围绕核心设计理念**：重构或修改代码时，**必须围绕 `Memories.md` 来创建无脑实施方案**，因为 `Memories.md` 记录了整个应用的核心设计理念与全局记忆。严禁碎片化、头痛医头脚痛医脚地盲目创建实施方案。
2. **强制事先读取设计理念**：在创建任何无脑实施方案（Implementation Plan）之前，**必须首先完整读取并深入分析 `Memories.md`**，只有从全局系统架构的视角审视并充分理解整个应用的设计理念之后，才可以去创建无脑实施方案，确保所有新方案与整体设计理念 100% 契合融合。
