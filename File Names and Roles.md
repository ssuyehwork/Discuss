...
本文档记录 QuarkMeta 项目中自身源码目录下所有真实存在的代码文件及其明确、可验证的职责描述、僵尸代码排查结果以及职责单一性 (SRP) 排查结果（已排除第三方 LibTIFF）。
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 单一文件中集中定义并实现了 Move, Rename, Metadata, SecureDelete, Encrypt, BatchRename 等多种完全不同领域和物理特性的 Command 操作类，属于命令类集合文件而非单一类定义文件
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 管理类既负责内存 256 分片元数据镜像维护，又直接包含 Windows WinAPI 文件属性提取、盘符卷序列号获取以及全量标签批处理等多样化职责
...
- **文件职责**：颜色选择与色相环交互控件。
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 定义了 ColorPicker, SvPicker, ColorStripPicker, HueSlider 多个不同功能的拾色器子控件，未能物理拆分
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 作为主内容视图面板，同时承担了右键上下文菜单构建、剪贴板/拖拽放下处理、QuickLook 预览代理触发以及多种数据源类型切换等多重职责
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 作为第五栏筛选面板，内嵌并直接管理了 `StyledCheckBox`, `ClickableRow` 以及下拉搜索历史面板的创建与事件转发
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 定义了 FramelessDialog, FramelessMessageBox, FramelessInputDialog, FramelessConfirmDialog, FramelessColorPicker 多个不同用途的弹窗类，组件定义杂糅
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 作为主窗口类，同时承担了 5 栏视图布局组装、托盘与顶栏菜单绑定、全局快捷键捕获响应及信号桥接等多重调度职责
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 头文件/源文件中杂糅定义了独立控件 `FileNameLineEdit` 与 Delegate 绘制类 `ThumbnailDelegate`，承担了文本高亮编辑与视图卡片渲染双重职责
...
- **僵尸代码**：无
- **职责单一性**：否（不单一） - 在同一个头文件中内联实现了树形卡片绘制、各种状态图标定位绘制、全行背景高亮以及针对键盘事件的捕获与行内编辑器管理，职责过于繁重
