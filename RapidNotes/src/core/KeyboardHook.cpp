...
LRESULT CALLBACK KeyboardHook::HookProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION) {
        ...
        // [NEW] 2026-05-27 按照用户要求：物理级拦截 Ctrl+Shift+A (显示全部数据) 防止第三方软件抢占
        if (pKey->vkCode == 'A' && isKeyDown) {
            bool ctrl = (GetKeyState(VK_CONTROL) & 0x8000);
            bool shift = (GetKeyState(VK_SHIFT) & 0x8000);
            bool alt = (GetKeyState(VK_MENU) & 0x8000);
            ...
            if (ctrl && shift && !alt) {
                ...
                if (foregroundPid == GetCurrentProcessId()) {
                    QMetaObject::invokeMethod(&KeyboardHook::instance(), "showAllDataRequested", Qt::QueuedConnection);
                    return 1; // 强制拦截，阻止传递给系统和其他软件
                }
            }
        }
