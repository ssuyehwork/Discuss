...
private:
    explicit QuickPreview(QWidget* parent = nullptr) : QWidget(parent, Qt::Tool | Qt::FramelessWindowHint) {
        ...
        addMetaRow("star_filled",  "评级",   m_metaRating);
...
public:
    ...
    void showPreview(const QVariantMap& note, const QPoint& pos, const QString& catName = "", QWidget* caller = nullptr) {
        ...
        if (m_metaRating) {
            ...
            if (r > 0) {
                // 2026-05-xx 按照用户要求：升级为标准的 5 星评级体系渲染
                // 获取实心与空心星标的 Base64 数据
                QPixmap fPix = IconHelper::getIcon("star_filled", "#FFD700", 16).pixmap(16, 16);
                QByteArray fBa; QBuffer fBuf(&fBa); fBuf.open(QIODevice::WriteOnly); fPix.save(&fBuf, "PNG");
                QString fBase64 = fBa.toBase64();

                QPixmap ePix = IconHelper::getIcon("star", "#444444", 16).pixmap(16, 16);
                QByteArray eBa; QBuffer eBuf(&eBa); eBuf.open(QIODevice::WriteOnly); ePix.save(&eBuf, "PNG");
                QString eBase64 = eBa.toBase64();

                QString html = "<div>";
                for(int i=0; i<5; ++i) {
                    QString imgData = (i < r) ? fBase64 : eBase64;
                    html += QString("<img src='data:image/png;base64,%1' width='16' height='16' style='margin-right:2px;'>").arg(imgData);
                }
                html += "</div>";
                m_metaRating->setText(html);
            } else {
                m_metaRating->setText("无");
                m_metaRating->setStyleSheet("color: #555; font-size: 12px; font-weight: normal;");
            }
        }
        if (m_metaStatus) {
            // 2026-05-xx 按照用户要求：物理级拔除 Emoji 符号，统一使用 SVG 图标 + 文字
            QStringList htmlParts;
            if (note.value("is_pinned").toInt()) {
                QPixmap pix = IconHelper::getIcon("pin_vertical", "#FF551C", 14).pixmap(14, 14);
                QByteArray ba; QBuffer buf(&ba); buf.open(QIODevice::WriteOnly); pix.save(&buf, "PNG");
                htmlParts << QString("<img src='data:image/png;base64,%1' width='14' height='14'> 置顶").arg(QString(ba.toBase64()));
            }
            if (note.value("is_favorite").toInt()) {
                QPixmap pix = IconHelper::getIcon("star_filled", "#F39C12", 14).pixmap(14, 14);
                QByteArray ba; QBuffer buf(&ba); buf.open(QIODevice::WriteOnly); pix.save(&buf, "PNG");
                htmlParts << QString("<img src='data:image/png;base64,%1' width='14' height='14'> 收藏").arg(QString(ba.toBase64()));
            }
            
            if (htmlParts.isEmpty()) {
                m_metaStatus->setText("常规");
            } else {
                m_metaStatus->setText("<html><body style='color:#ddd; font-size:12px;'>" + htmlParts.join(" &nbsp;&nbsp; ") + "</body></html>");
            }
        }
...
protected:
    ...
    void copyFullContent() {
        ...
        if (!textToCopy.isEmpty()) {
            ...
            ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #2ecc71;'>已复制到剪贴板</b>", 700, QColor("#B0B0B0"), ToolTipOverlay::ActivePriority);
        }
    }
