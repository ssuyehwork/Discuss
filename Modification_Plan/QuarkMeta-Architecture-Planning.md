...
#### A. 磁盘目录模式下：
一、3 栏核心主功能区（从左到右）
1. **第一栏（最左侧）：目录导航栏（Dir Tree Navigation / Col 0）**
   - 包含“此电脑”、本地盘符（C/D/E/Z盘等）、桌面等标准的本地磁盘目录树结构。
   - 物理锁定宽度，`stretchFactor` 必须硬性写死为 `0`。

2. **第二栏（中间）：收藏夹独占栏（Dedicated Favorites Bar / Col 1）**
   - 独占一整栏，专门展示收藏的常用快捷文件夹，垂直贯通。
   - 物理锁定宽度（如 200px），`stretchFactor` 必须硬性写死为 `0`，严禁设为 1，绝对不允许出现拉伸膨胀霸占中间空白区的情况。

3. **第三栏（主视图区）：内容展示区（Content Panel / Col 2）**
   - 展示当前文件夹内的图片/文件缩略图网格或列表视图。
   - 为全界面**唯一核心主视图拉伸区**，`stretchFactor` 必须硬性写死为 `1`，窗口最大化或拉伸时仅此栏扩张。

二、2 栏右侧辅助工具栏（可按需展开/收起）
4. **第四栏：元数据属性栏（Meta Panel / Col 3）**
   - 展示和编辑当前选中文件的评级（星标）、颜色标记、标签、备注、尺寸等元数据。
   - 物理锁定宽度，`stretchFactor` 必须硬性写死为 `0`。

5. **第五栏（最右侧）：条件筛选栏（Filter Panel / Col 4）**
   - 提供按颜色、文件类型、评级、时间等维度快速过滤当前内容的筛选工具。
   - 物理锁定宽度，`stretchFactor` 必须硬性写死为 `0`。

三、物理分割手柄与样式对齐（Splitter Handles & Styling Specification）
- **5 像素物理缝隙**：主拆分条 `m_mainSplitter` 必须显式设置 `setHandleWidth(5)`，每个栏区之间必须拥有统一、清晰的 5 像素分割手柄。
- **QSS 选择器对齐**：样式表中必须包含 `#FavoriteContainer`，确保第二栏与 `SidebarContainer`、`ListContainer` 等统一保持 1px 边框与透明背景，绝不遗漏或残缺。
