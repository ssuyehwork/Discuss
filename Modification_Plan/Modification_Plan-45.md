# 恢复侧边栏“分类”外壳容器与手动和自动分类总计数 —— Modification_Plan-45.md

> 状态：待批准执行（尚未获得用户"批准执行"指令）

## 1. 任务背景
本方案承接自 `Modification_Plan-44.md`，因用户提出最新的修正要求：
- 将原本设计的“分类”顶级容器分组上显示的计数，修改为统计**所有手动和自动创建的分类数量**（即排除了顶级托管库本身之后，剩下的全部自定义分类的总个数）。
旧方案 `Modification_Plan-43.md` 与 `Modification_Plan-44.md` 均永久作为历史只读保留。所有的技术演进与施工方案在本篇 `Modification_Plan-45.md` 中进行完整细化与最终规范，确保功能时序高内聚，数据流纯净不污染数据库。

---

## 2. 问题定位
- 模型构建端 `CategoryModel::refresh()` 需要在构建“分类”根节点容器前，先遍历 `categories` 列表。
- 只要物理分类不是顶级托管库本身（即满足 `!(cat.parentId == 0 && QString::fromStdWString(cat.name).startsWith("ArcMeta.Library_"))`），则将其判定为手动或自动创建的分类。
- 统计出的满足上述条件的总数量计入 `userTotalCount`。
- 构建 `QStandardItem("分类")` 时，使用 `QString("分类 (%1)").arg(userTotalCount)` 动态渲染标题，绑定 `NameRole = "分类"`。
- 在 `CategoryPanel.cpp` 以及 `CategoryFilterProxyModel.h` 中，继续使用 `"分类"` 的匹配保护与数据路由。
...
| 3 | 分类上显示的计数计的是手动和自动创建的分类数量 | 计算所有非顶级托管库本身的自定义分类总数（即包含所有下属自定义分类），动态刷新到分类标题上。 | ✅ |
...
在 `refresh` 函数中，先统计所有非顶级托管库本身（手动或自动创建）的分类总数量 `userTotalCount`，然后用 `分类 (数量)` 作为节点名称创建 userGroup 顶级项。并在 `dropMimeData` 校验中放行对该节点的拖入动作。
...
**本次方案涉及范围：**
- [ ] `src/ui/CategoryModel.cpp`：添加 `userGroup` 作为“分类”创建，并统计除了顶级托管库本身（即手动和自动创建的所有分类）之外的分类数量 `userTotalCount`，并在挂载和拖放逻辑中对齐使用 `"分类"`。
...
## 6. 实现准则与预警【核心】
1. **精准计数安全**：在统计手动和自动创建的分类数量时，仅需通过 `!(cat.parentId == 0 && QString::fromStdWString(cat.name).startsWith("ArcMeta.Library_"))` 排查顶级托管库节点。这包含了所有通过界面手动新建的，以及拖入文件夹或扫描自动生成的子孙分类，使计算在 1行代码的低复杂度下实现完美的性能和百分之百的业务准确度。
