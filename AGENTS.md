...
3. **强制精准对标与复用已有代码（严禁凭空脑补、残余命名与重复造轮子）**：在编写任何实施方案之前，**必须先查阅、检索并核对现有项目代码库中真实存在的类定义、静态/单例 API 签名（如 `DatabaseManager::instance().getGlobalDb()`）、结构体成员和作用域变量**。严禁凭空捏造不存在的函数/方法（如凭空脑补 `getGlobalDatabaseHandle()`），严禁使用历史重构过程残留的未落盘标识符，必须坚决复用已有项目真实架构与精确 API 签名，杜绝任何低级的凭空脑补、残余命名与重复造轮子！
