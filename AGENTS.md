...
5. **强制公有契约与下游符号关联排查（公有 API 保护与契约锁）**：在剥离、重构或迁移某个类的内部类型（如枚举、结构体、类型定义）时，**必须提前进行全局符号关联检索**（如检索 `ClassA::EnumName` 与 `ClassA::EnumValue`），排查所有外部调用方与下游组件。重构时严禁直接删除原类的公有类型定义与枚举常数，**必须通过 `using` 类型别名与 `static constexpr` 常量转接 100% 保留原有公有契约**，并在实施方案中完整覆盖下游引用文件的适配补丁。
...
### 5.6 实施方案严格自检流程 (Mandatory Plan Self-Check Routine)
创建实施方案文档后，**必须对照以下标准自检项进行逐一核对自查**：
- [ ] **公有契约自检**：重构或迁移类型时，原类的公有类型定义与枚举常量是否已通过 `using` / `static constexpr` 100% 保留？下游调用方是否仍可编译通过？
- [ ] **代码搜索自检**：SEARCH 块中的代码是否通过全局检索确认在仓库中 100% 存在且前后上下文完全精准？
- [ ] **CMakeLists 注册自检**：新创建的 `.h` 与 `.cpp` 文件是否均已包含在 `CMakeLists.txt` 的注册 Diff 块中？
- [ ] **文件只读命名自检**：方案文件名是否严格对应核心类名（且为英文），后续补充更新是否采用了递增后缀（如 `-1.md`）？
