...
3. **纯 SVG 矢量图标与动态配色渲染规范**：收藏夹列表视图条目禁止使用 Shell 位图图标，强制统一使用 `UiHelper::getIcon(iconKey, color, 18)` 渲染视网膜屏高保真矢量 SVG 图标，默认使用金色（`#FDB70A`）。
4. **上下文菜单图标与调色板切换规范**：收藏夹右键上下文菜单内置 `ColorStripPicker` 调色板与 `iconMenu` 矢量图标选择器，改动实时刷盘至 `global.db` 并触发 Model 重绘。
