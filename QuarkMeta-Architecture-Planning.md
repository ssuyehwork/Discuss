...
1. **主窗口 (MainWindow) 拆分与 TitleBar 交互规范**：主窗口仅允许承载顶层 UI 布局构建与 QSS 样式加载。无边框窗口边缘 Hover 光标改变全权交由 `ResizeEventFilter` 统一维护，严禁在 `MainWindow` 内部重复编写光标判定逻辑（方案 A 架构拆分）。顶层无边框标题栏（TitleBar）的双击最大化/还原与按住拖拽跟随交互，必须物理解耦并完全交由独立的专属事件过滤器 `TitleBarEventFilter` 承载，严禁在 `MainWindow::eventFilter` 中混杂堆砌标题栏拖拽逻辑。全局快捷键捕获与事件分发必须解耦至 `GlobalShortcutController`；多面板之间的联动逻辑解耦至 `PanelMediator`。
